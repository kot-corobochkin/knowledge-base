...
df_clean = df.dropna(subset=["profit_per_worker", "avg_workers_per_enterprise", "profitability_ratio"])
...
fig = px.scatter(
    ...
    labels={
        ...
        "profitability_ratio": "Прибыль на каждый юань продукции"
    },
    hover_data={
        "Sector": False,  # не дублировать в теле
        "avg_workers_per_enterprise": True,
        "profit_per_worker": True,
        "profitability_ratio": True,
    },
    title="Уязвимость и прибыльность отраслей"
)


df["margin_on_sales"] = df["Total Profits"] / df["Business Revenue"]
df["margin_on_cost"] = df["Total Profits"] / df["Business Cost"]
df["return_on_assets"] = df["Total Profits"] / df["Total Assets"]
df["profit_per_worker"] = df["Total Profits"] / df["Employed Persons"]
df["return_on_working_capital"] = df["Total Profits"] / df["Working Capital"]
...
# Топ-10 по рентабельности продаж
top_profitable = df_clean.sort_values(by="margin_on_sales", ascending=False).head(10)

# Запуск Dash
app = dash.Dash(__name__)

app.layout = html.Div([
    html.H4('Наиболее прибыльные отрасли (по рентабельности продаж)'),
    html.P(id='table_out'),
    dash_table.DataTable(
        id='table',
        columns=[{"name": col, "id": col} for col in top_profitable.columns],
        data=top_profitable.to_dict('records'),
    ...
    html.Br(),
    html.H4("Уязвимость и прибыльность отраслей"),
    dcc.Graph(figure=fig)  # <-- ЭТО добавит график
])
